...
export async function POST(req: NextRequest) {
  ...
  const { deviceId, candidateId, entropy, transitions, rle, score, signature } =
  ...
  if (!candidateId || entropy == null || transitions == null || rle == null || score == null) {
  ...
  console.log(
    `[validation-result] device=${deviceId} candidate=${candidateId} esp=${espScore.toFixed(3)} server=${serverScore.toFixed(3)} drift=${drift.toFixed(3)}`
  );

  if (drift > 0.4) {
    console.warn(`[validation-result] drift élevé device=${deviceId} candidate=${candidateId} drift=${drift.toFixed(3)}`);
  }
  ...
  console.log(
    `[validation-result] vote device=${deviceId} votes=${voteCount}/${needed} quorum=${quorumReached}`
  );
  ...
  if (quorumReached) {
    ...
    Promise.all(poolMembers.map((dId) => redis.del(`personal:frame:${dId}`)))
      .catch((err) => console.error("[validation-result] clear personal frames error:", err));

    console.log(
      `[validation-result] BLOC #${block.blockIndex} finalisé hash=${block.blockHash.slice(0, 12)}... display=${block.displayTime}s`
    );
