...
export default function DrawIndexPage() {
  ...
  const handleGo = () => {
    ...
    router.push(`/draw/${selectedDevice.id}/${selectedScreen}`);
  };
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "60vh", gap: "1rem" }}>
        ...
        <a href="/onboard" style={{
          padding: "0.7rem 1.5rem", borderRadius: "8px", background: "var(--accent)",
    ...
    <div style={{ maxWidth: 600, margin: "0 auto", padding: "2rem 1rem" }}>
      ...
      <div style={{ marginBottom: "1.5rem" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
          {devices.map(d => (
            <button key={d.id} onClick={() => { setSelectedDevice(d); setSelectedScreen(""); }}
              style={{
                display: "flex", alignItems: "center", gap: "1rem", padding: "0.875rem 1rem",
                borderRadius: "8px", cursor: "pointer", textAlign: "left", width: "100%",
                border: `1px solid ${selectedDevice?.id === d.id ? "var(--accent)" : "var(--border)"}`,
                background: selectedDevice?.id === d.id ? "rgba(124,107,255,0.08)" : "var(--bg2)",
                color: "var(--text)", transition: "all 0.15s"
              }}>
              <div style={{ width: 8, height: 8, borderRadius: "50%", background: d.lastPing ? "var(--success)" : "var(--text3)", flexShrink: 0 }} />
              <div>
                <div style={{ fontWeight: 600 }}>{d.name}</div>
                <div style={{ color: "var(--text3)", fontSize: "0.75rem", fontFamily: "JetBrains Mono, monospace", marginTop: "0.1rem" }}>
                  {d.ip}:{d.port} · {d.screens.length} écran(s)
                </div>
              </div>
        ...
        <div style={{ marginBottom: "2rem" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
            {selectedDevice.screens.map(sid => {
              ...
              return (
                <button key={sid} onClick={() => setSelectedScreen(sid)}
                  style={{
                    display: "flex", alignItems: "center", gap: "1rem", padding: "0.875rem 1rem",
                    borderRadius: "8px", cursor: "pointer", textAlign: "left", width: "100%",
                    border: `1px solid ${selectedScreen === sid ? "var(--accent2)" : "var(--border)"}`,
                    background: selectedScreen === sid ? "rgba(255,107,157,0.08)" : "var(--bg2)",
                    color: "var(--text)", transition: "all 0.15s"
                  }}>
                  <div>
                    <div style={{ fontWeight: 600 }}>{p.name}</div>
                    <div style={{ color: "var(--text3)", fontSize: "0.75rem", marginTop: "0.1rem" }}>{p.description}</div>
                  </div>
      ...
      <button
        ...
        style={{
          ...
          background: (!selectedDevice || !selectedScreen) ? "var(--bg3)" : "var(--accent)",
          color: (!selectedDevice || !selectedScreen) ? "var(--text3)" : "#fff",
          border: "1px solid var(--border)", cursor: (!selectedDevice || !selectedScreen) ? "not-allowed" : "pointer",
