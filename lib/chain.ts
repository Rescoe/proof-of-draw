...
export interface Candidate {
  ...
  drawingHash:  string;
  score:        number;   // pré-calculé côté serveur
  submittedAt:  number;
  expiresAt:    number;   // timestamp — candidat expiré si non validé
  poolSize:     number;   // nb d'ESP actifs dans la pool au moment de la soumission
}
...
export interface ValidationVote {
  ...
  rle:         number;
  score:       number;
  signature:   string;   // "deviceId:candidateId:score" signé (simplifié pour V1)
  votedAt:     number;
}

export interface VoteMap {
  candidateId: string;
  votes:       Record<string, ValidationVote>;  // deviceId → vote
}
...
const CANDIDATE_TTL_SEC = parseInt(process.env.CANDIDATE_TTL_SEC ?? "600"); // 10min pour valider
...
/**
 * Enregistre un vote d'un ESP validateur.
 * Retourne true si le quorum est atteint après ce vote.
 */
export async function castVote(
  ...
  // Quorum minimal absolu : au moins 1 vote (réseau solo possible)
  const quorumReached = voteCount >= Math.max(1, needed);

  return { quorumReached, voteCount, needed };
}

// ─── Finalisation d'un bloc ───────────────────────────────────────────────────

/**
 * Crée et persiste un nouveau bloc à partir d'un candidat validé.
 * Retourne le bloc créé.
 */
export async function finalizeBlock(
  ...
  // Score final = moyenne des votes (robuste aux outliers)
  const avgScore = votes.reduce((s, v) => s + v.score, 0) / votes.length;
  const finalScore = (candidate.score + avgScore) / 2; // moyenne serveur + validateurs

  const displayTime = computeDisplayTime(finalScore, parentHash);

  // Contenu canonique du bloc pour le hash
  ...
  // Persistance : head + archive par hash + index
  await Promise.all([
    redis.set(KEY_HEAD,           JSON.stringify(block)),         // permanent — la head n'expire pas
    redis.set(blockKey(blockHash), JSON.stringify(block), { ex: 30 * 24 * 3600 }), // 30 jours
    redis.set(indexKey(length),   blockHash, { ex: 30 * 24 * 3600 }),
    redis.set(KEY_LENGTH,         String(length + 1)),
  ]);

  console.log(`[chain] BLOC #${length} hash=${blockHash.slice(0, 12)}... score=${finalScore.toFixed(3)} display=${displayTime}s`);

  return block;
}

// ─── Infos résumées pour les ESP ──────────────────────────────────────────────
// L'ESP reçoit ça après chaque pull pour synchroniser son état de chaîne

export interface ChainSummary {
  blockIndex:   number;
  blockHash:    string;  // les ESP stockent ça en EEPROM
  displayTime:  number;
  artistName:   string;
  poolScreen:   string;
  minedAt:      number;
